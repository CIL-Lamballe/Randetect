...
pub fn create(to_address: &str, subject_text: &str, body_text: &str){

    let from_address = from_address.to_string();
    let from = "NAS_Alert".to_string() + "\n";
...
pub fn send (to_address: &str) {
    ...
    // Send Email using ssmtp -t
    let to_address = to_address.to_string();
    let ssmtp_sh_frist_half = "ssmtp ".to_string() + &to_address;
    let ssmtp_sh_second_half = " < email.txt".to_string();
    let ssmtp_sh = ssmtp_sh_frist_half + & ssmtp_sh_second_half;
